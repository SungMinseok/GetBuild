...
class UpdateChecker:
    ...
    def check_for_updates(self) -> Tuple[bool, Optional[Dict[str, Any]], Optional[str]]:
        ...
        try:
            ...
            try:
                # 버전 형식 정규화 (하이픈을 점으로 변경)
                current_normalized = self.current_version.replace('-', '.')
                latest_normalized = latest_version.replace('-', '.')
                
                if version_parser.parse(latest_normalized) > version_parser.parse(current_normalized):
                    download_url = None
                    for asset in release_data.get('assets', []):
                        if asset['name'].endswith('.zip') and 'QuickBuild' in asset['name']:
                            download_url = asset['browser_download_url']
                            break

                    if not download_url:
                        return False, None, "다운로드 가능한 QuickBuild ZIP 파일이 없습니다."
...
class UpdateDownloader:
    ...
    def download(self, url: str, progress_callback=None) -> Optional[str]:
        ...
        try:
            ...
            download_path = os.path.join(base_dir, "QuickBuild_update.zip")
...
class UpdateInstaller:
    ...
    def install_update(zip_path: str, restart: bool = True, logger=None) -> bool:
        ...
        try:
            ...
            # 실행 파일 경로
            current_exe = sys.executable if getattr(sys, 'frozen', False) else os.path.abspath("QuickBuild.exe")
...
chcp 65001 > nul
echo ========================================
echo QuickBuild Update Installation...
echo ========================================
